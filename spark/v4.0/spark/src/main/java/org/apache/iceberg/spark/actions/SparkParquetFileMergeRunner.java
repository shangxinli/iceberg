...
public class SparkParquetFileMergeRunner extends SparkBinPackFileRewriteRunner {
  ...
  protected void doRewrite(String groupId, RewriteFileGroup group) {
    ...
    MessageType schema = canUseMerger(group);
    if (schema == null) {
      LOG.info(
   ...
   */
  private MessageType canUseMerger(RewriteFileGroup group) {
   ...
   */
  private void mergeParquetFilesDistributed(String groupId, RewriteFileGroup group, MessageType schema) {
    ...
    OutputFileFactory fileFactory =
        OutputFileFactory.builderFor(table(), spec.specId(), 1)
