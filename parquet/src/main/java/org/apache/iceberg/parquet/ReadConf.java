...
class ReadConf<T> {
  ...
  ReadConf(InputFile file, ParquetReadOptions options, Schema expectedSchema, Expression filter,
    ...
    for (int i = 0; i < shouldSkip.length; i += 1) {
      ...
      /**
       * The pageStatsFilter here makes sure the call to readNextFilteredRowGroup() in ParquetReader#advance() won't
       * advance RowGroups without IceBerg's knowledge.
       *
       * TODO: Can we remove blockStatsFilter since pageStatsFilter can do the work at finer-grain? Let's keep it
       * for two reasons.
       *   1) If the data is written using parquet-mr with the older version than 1.11.0, no ColumnIndex feature available.
       *   2) To use ColumnIndex, we need reader to read the page statistics from the disk. 'blockStatsFilter' can optimize
       *      it a little when it returns 'false'.
       */
      boolean shouldRead = filter == null || (
          blockStatsFilter.shouldRead(typeWithIds, rowGroup) &&
              // pageStatsFilter.shouldRead(typeWithIds, rowGroup) &&
